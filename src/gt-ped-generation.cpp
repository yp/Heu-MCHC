...
// Check parameters
  ...
  unsigned int n= 0; // no of individuals

  vector<p_hap_t> indivs;

  list<int> indivs_as_p_founder;
  list<int> indivs_as_m_founder;
  list<int> indivs_as_children;
// Topology creation
// first individual
  indivs.push_back(p_hap_t(n, len_hap));
  ++n;
  indivs_as_children.push_back(0);
  indivs_as_p_founder.push_back(0);

  while (n<n_indiv) {
// create a family given a child
	 if (n<n_indiv) {
		int c= indivs_as_children.front();
		indivs_as_children.pop_front();
		int f= n;
		int m= n+1;
		n= n+2;
		indivs.push_back(p_hap_t(f, len_hap));
		indivs.push_back(p_hap_t(m, len_hap));
		indivs[c].fid= f;
		indivs[c].mid= m;
		indivs_as_children.push_back(f);
		indivs_as_children.push_back(m);
		double pc= p_children;
		while ((n<n_indiv) && (rnd_01()<pc)) {
		  indivs.push_back(p_hap_t(n, len_hap));
		  indivs[n].fid= f;
		  indivs[n].mid= m;
		  if (rnd_01()<0.5)
			 indivs_as_p_founder.push_back(n);
		  else
			 indivs_as_m_founder.push_back(n);
		  ++n;
		  pc= pc*p_children;
		}
	 }
	 if (n<n_indiv && !indivs_as_p_founder.empty()) {
		int f= indivs_as_p_founder.front();
		if (rnd_01()>=p_multimating)
		  indivs_as_p_founder.pop_front();
		int m= n;
		int c= n+1;
		n= n+2;
		indivs.push_back(p_hap_t(m, len_hap));
		...
		indivs_as_children.push_back(m);
		if (rnd_01()<0.5)
		  indivs_as_p_founder.push_back(c);
		else
		  indivs_as_m_founder.push_back(c);
		double pc= p_children;
		while ((n<n_indiv) && (rnd_01()<pc)) {
		  indivs.push_back(p_hap_t(n, len_hap));
		  indivs[n].fid= f;
	 ...
	 if (n<n_indiv && !indivs_as_m_founder.empty()) {
		int m= indivs_as_m_founder.front();
		if (rnd_01()>=p_multimating)
		  indivs_as_m_founder.pop_front();
		int f= n;
		int c= n+1;
		n= n+2;
		indivs.push_back(p_hap_t(f, len_hap));
