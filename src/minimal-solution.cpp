...
// i is not a founder
  ...
  INFO_IF(first, "The linear system has " ST_FMTL(5) "  variables and " ST_FMTL(5) "  equations.", nvar, neq);
...
// i is not a founder
		...
		for (unsigned int l= 0; l<gp->n_loci; ++l) {
		  DEBUG("Individual %4d,  locus %4d", i, l);
		  if (is[i]->f->g[l]==GEN2) {
  ...
  const size_t rank= mzd_echelonize_m4ri(A, 1, 0);
  const size_t rank2= mzd_echelonize_m4ri(Aext, 1, 0);

  INFO_IF(first, "Rank " ST_FMT "", rank);
  if (rank != rank2) {
	 ERROR_IF(first, "Rank of coefficient matrix and complete matrix differ.");
	 return false;
  }
  for (size_t i= 0; i<neq; ++i) {
  ...
  for (int i= rank-1; i>=0; --i) {
	 ...
	 while (mzd_read_bit(A, i, first_one)==0)
		++first_one;
	 const int kt= mzd_read_bit(B, i, 0);
	 for (int j= 0; j<=i; ++j) {
	 ...
	 for (unsigned int l= 0; l<gp->n_loci; ++l) {
		...
		bool c3= (is[i]->g[l]!=GEN1 ||
					 ((is[i]->hv_f[l]==is[i]->hv_m[l]) &&
					  (is[i]->hv_f[l]==1)));
		if (!(c1 && c2 && c3)) {
		  ERROR_IF(first, "Genotype-haplotype consistency violated in individual "
				  "%5d at locus %5d.", i, l);
		  return false;
		}
	 }
...
check_inheritance(pgenped gp) {
  ...
  for (unsigned int i= 0; i<gp->n_indiv; ++i) {
	 if (is[i]->fi!=-1) {
		DEBUG("Analysis individual %5d.", i);
		...
		bool mat= is_present(is[i]->hv_m, is[i]->m->hv_f, is[i]->m->hv_m, gp->n_loci,
									is[i]->dm, is[i]->rm);
		if (!pat) {
		  ERROR_IF(first, "Paternal haplotype of individual %5d is not inherited.", i);
		  return false;
		}
		if (!mat) {
		  ERROR_IF(first, "Maternal haplotype of individual %5d is not inherited.", i);
		  return false;
		}
	 }
  }
...
int main(int argc, char ** argv) {
  ...
  FILE * fped= fopen(args_info.pedigree_arg, "r");
  if (fped==NULL) {
	 FATAL("Opening file \"%s\" failed.", args_info.pedigree_arg);
	 fail();
  }
  pgenped orig_gp= gp_read_from_file(fped);
  fclose(fped);

  ped::e_vars_t evs;
  read_events_from_file(fev, evs, orig_gp->n_indiv, orig_gp->n_loci);
  fclose(fev);

  first= true;
  pgenped gp= gp_copy(orig_gp);
  integrate_events_in_pedigree(gp, evs);
  bool complete= solve_pedigree(gp);
  complete= complete && compute_haplotypes(gp);
  complete= complete && check_inheritance(gp);
  gp_destroy(gp);
  if (!complete) {
	 FATAL("The complete solution is NOT a solution!");
	 fail();
  }

  first= false;

  std::vector<bool> mask(evs.size(), false);
  size_t removed= 0;

  for (size_t i= 0; i<mask.size(); ++i) {
	 INFO("Trying to remove event " ST_FMT "...", i);
	 mask[i]= true;
	 ped::e_vars_t newevs;
	 size_t j= 0;
	 for (ped::e_vars_t::const_iterator it= evs.begin();
	 ...
	 // 	fail();
	 // }
	 // gp= gp_read_from_file(fped);
	 gp= gp_copy(orig_gp);
	 integrate_events_in_pedigree(gp, newevs);
	 bool ris= solve_pedigree(gp);
	 ris= ris && compute_haplotypes(gp);
	 ris= ris && check_inheritance(gp);
	 gp_destroy(gp);
	 mask[i]= ris;
	 if (ris) {
		++removed;
  ...
  INFO("Removed " ST_FMT " events.", removed);
  printf("= Minimal solution obtained from pedigree %s "
			"and events %s after removing " ST_FMT " unnecessary events\n",
			args_info.pedigree_arg, args_info.events_arg,
			removed);
  size_t i= 0;
  for(ped::e_vars_t::const_iterator it= evs.begin();
		it != evs.end(); ++it, ++i) {
	 if (!mask[i]) {
		printf("= %s %4d %4d %2d\n", ped::kind_names[(*it).kind()],
			  (*it).i, (*it).l, (*it).p);
	 }
  }

  cmdline_parser_free(&args_info);
  INFO("MIN-SOL terminated.");
  return 0;
}
